...
public class GoogleMapTile : MonoBehaviour
{
    ...
    IEnumerator _RefreshMapTile()
    {
        ...
        tileCenterLocation.Latitude = GoogleMapUtil.AdjustLatByPixels(worldCenterLocation.Latitude, (int)(size * 1 * TileOffset.y), zoomLevel);
        tileCenterLocation.Longitude = GoogleMapUtil.AdjustLonByPixels(worldCenterLocation.Longitude, (int)(size * 1 * TileOffset.x), zoomLevel);
        ...
        //set map bounds rect
        TopLeftCorner.x = GoogleMapUtil.AdjustLonByPixels(tileCenterLocation.Longitude, -size, zoomLevel);
        TopLeftCorner.y = GoogleMapUtil.AdjustLatByPixels(tileCenterLocation.Latitude, size, zoomLevel);

        BottomRightCorner.x = GoogleMapUtil.AdjustLonByPixels(tileCenterLocation.Longitude, size, zoomLevel);
        BottomRightCorner.y = GoogleMapUtil.AdjustLatByPixels(tileCenterLocation.Latitude, -size, zoomLevel);
